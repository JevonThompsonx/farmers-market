...
import { PATCH as farmPATCH, DELETE as farmDELETE } from "@/app/api/farms/[id]/route";
...
async function callWithSession(handler: (req: NextRequest, ctx: { params: Promise<{ id: string }> }) => Promise<Response>, url: string, body: unknown, opts: { method: string; id?: string }) {
  ...
  const ctx = { params: Promise.resolve({ id: opts.id ?? "x" }) } as { params: Promise<{ id: string }> };
...
beforeEach(() => {
  ...
  vi.mocked(getProductById).mockResolvedValue({ ...mockProduct, farmId: "farm-1" });
...
describe("anonymous mutations are rejected", () => {
  it("POST /api/farms -> 401", async () => {
    ...
    const res = await farmsPOST(makeRequest("http://localhost:3000/api/farms", {
      method: "POST",
      body: JSON.stringify({ name: "X", city: "Y", state: "OR", description: "d" }),
      headers: { "Content-Type": "application/json" },
  ...
  it("POST /api/products -> 401", async () => {
    vi.mocked(getUserId).mockRejectedValue(new UnauthorizedError());
    const res = await productsPOST(makeRequest("http://localhost:3000/api/products", {
      method: "POST",
      body: JSON.stringify({ name: "X", price: 1, description: "d", category: "vegetables", farmId: "farm-1" }),
      headers: { "Content-Type": "application/json" },
    }), { params: Promise.resolve({}) });
    expect(res.status).toBe(401);
  ...
  it("PATCH /api/farms/[id] -> 401", async () => {
    vi.mocked(getUserId).mockRejectedValue(new UnauthorizedError());
    const res = await callWithSession(farmPATCH, "http://localhost:3000/api/farms/farm-1", { name: "X" }, { method: "PATCH", id: "farm-1" });
    expect(res.status).toBe(401);
  ...
  it("DELETE /api/farms/[id] -> 401", async () => {
    vi.mocked(getUserId).mockRejectedValue(new UnauthorizedError());
    const res = await callWithSession(farmDELETE, "http://localhost:3000/api/farms/farm-1", undefined, { method: "DELETE", id: "farm-1" });
    expect(res.status).toBe(401);
  ...
  it("PATCH /api/products/[id] -> 401", async () => {
    vi.mocked(getUserId).mockRejectedValue(new UnauthorizedError());
    const res = await callWithSession(productPATCH, "http://localhost:3000/api/products/product-1", { name: "X" }, { method: "PATCH", id: "product-1" });
    expect(res.status).toBe(401);
  ...
  it("DELETE /api/products/[id] -> 401", async () => {
    vi.mocked(getUserId).mockRejectedValue(new UnauthorizedError());
    const res = await callWithSession(productDELETE, "http://localhost:3000/api/products/product-1", undefined, { method: "DELETE", id: "product-1" });
    expect(res.status).toBe(401);
  ...
  it("POST /api/farms/[id]/reviews -> 401", async () => {
    vi.mocked(getUserId).mockRejectedValue(new UnauthorizedError());
    const res = await farmReviewsPOST(makeRequest("http://localhost:3000/api/farms/farm-1/reviews", {
      method: "POST",
  ...
  it("POST /api/products/[id]/reviews -> 401", async () => {
    vi.mocked(getUserId).mockRejectedValue(new UnauthorizedError());
    const res = await productReviewsPOST(makeRequest("http://localhost:3000/api/products/product-1/reviews", {
      method: "POST",
      body: JSON.stringify({ body: "nice", rating: 5 }),
      headers: { "Content-Type": "application/json" },
...
describe("authenticated mutations are accepted and use the real user id", () => {
  it("POST /api/farms -> 201 with session ownerId", async () => {
    const res = await farmsPOST(makeRequest("http://localhost:3000/api/farms", {
      method: "POST",
      body: JSON.stringify({ name: "Sunrise", city: "Portland", state: "OR", description: "A lovely organic farm in the Pacific Northwest." }),
      headers: { "Content-Type": "application/json" },
    }), { params: Promise.resolve({}) });
    expect(res.status).toBe(201);
    expect(createFarm).toHaveBeenCalledWith(expect.objectContaining({ ownerId: OWNER }));
  });

  it("POST /api/products -> 201 and verifies farm ownership", async () => {
    const res = await productsPOST(makeRequest("http://localhost:3000/api/products", {
      method: "POST",
      body: JSON.stringify({ name: "Tomatoes", price: 4.99, description: "Ripe and juicy heirloom tomatoes grown without pesticides.", category: "vegetables", farmId: "farm-1" }),
      headers: { "Content-Type": "application/json" },
    }), { params: Promise.resolve({}) });
    expect(res.status).toBe(201);
  ...
  it("PATCH /api/farms/[id] -> 200", async () => {
    const res = await callWithSession(farmPATCH, "http://localhost:3000/api/farms/farm-1", { name: "New" }, { method: "PATCH", id: "farm-1" });
  ...
  it("PATCH /api/products/[id] -> 200", async () => {
    const res = await callWithSession(productPATCH, "http://localhost:3000/api/products/product-1", { name: "New" }, { method: "PATCH", id: "product-1" });
    expect(res.status).toBe(200);
  ...
  it("POST /api/farms/[id]/reviews -> 201 with session authorId", async () => {
    const res = await farmReviewsPOST(makeRequest("http://localhost:3000/api/farms/farm-1/reviews", {
      method: "POST",
      body: JSON.stringify({ body: "Fresh produce and friendly staff!", rating: 5 }),
      headers: { "Content-Type": "application/json" },
    }), { params: Promise.resolve({ id: "farm-1" }) });
    expect(res.status).toBe(201);
  ...
  it("POST /api/products/[id]/reviews -> 201 with session authorId", async () => {
    const res = await productReviewsPOST(makeRequest("http://localhost:3000/api/products/product-1/reviews", {
      method: "POST",
      body: JSON.stringify({ body: "Great quality and fast delivery!", rating: 5 }),
      headers: { "Content-Type": "application/json" },
    }), { params: Promise.resolve({ id: "product-1" }) });
    expect(res.status).toBe(201);
...
describe("a user cannot mutate another user's resource", () => {
  beforeEach(() => {
    ...
    (getFarmById as ReturnType<typeof vi.fn>).mockImplementation((id: string) => {
      if (id === "farm-1") return Promise.resolve({ ...mockFarm, ownerId: OTHER });
...
describe("public reads still work without auth", () => {
  it("GET /api/farms -> 200", async () => {
    const res = await farmsGET(makeRequest("http://localhost:3000/api/farms"), { params: Promise.resolve({}) });
    expect(res.status).toBe(200);
  });

  it("GET /api/products -> 200", async () => {
    const res = await productsGET(makeRequest("http://localhost:3000/api/products"), { params: Promise.resolve({}) });
    expect(res.status).toBe(200);
  });

  it("GET /api/farms/[id] -> 200", async () => {
    const res = await farmItemGET(makeRequest("http://localhost:3000/api/farms/farm-1"), { params: Promise.resolve({ id: "farm-1" }) });
    expect(res.status).toBe(200);
  });

  it("GET /api/products/[id] -> 200", async () => {
    const res = await productItemGET(makeRequest("http://localhost:3000/api/products/product-1"), { params: Promise.resolve({ id: "product-1" }) });
    expect(res.status).toBe(200);
  });

  it("GET /api/farms/[id]/reviews -> 200", async () => {
    const res = await farmReviewsGET(makeRequest("http://localhost:3000/api/farms/farm-1/reviews"), { params: Promise.resolve({ id: "farm-1" }) });
    expect(res.status).toBe(200);
  });

  it("GET /api/products/[id]/reviews -> 200", async () => {
    const res = await productReviewsGET(makeRequest("http://localhost:3000/api/products/product-1/reviews"), { params: Promise.resolve({ id: "product-1" }) });
    expect(res.status).toBe(200);
  });
});
