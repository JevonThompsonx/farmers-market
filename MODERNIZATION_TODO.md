...
- [x] **Missing dependency: `pino-pretty`.** `src/lib/logger.ts` configures `transport: { target: "pino-pretty" }`, but `pino-pretty` is **not** in `package.json` (`dependencies` or `devDependencies`). In dev this transport load will throw at runtime. Add `pino-pretty` as a devDependency (and pin it). *(Wave 1 fixed the *other* missing peer — `@testing-library/dom` — but `pino-pretty` is still absent. Still open.)* **→ DONE (Wave 2, P0): added `pino-pretty@^13.1.3` devDependency (bun.lock updated).**
