import { fruitList, vegetableList, milkDairyList, cheeseDairyList, liquidUnits, fruitUnits, cheeseUnits, cheeseForms, vegetableForms, fruitForms, milkDiaryForms, } from "./seedData/productData/index.js";
...
const productGenerator = async () => {
    const prodTypeArray = ["fruit", "vegetable", "milkDairy", "cheeseDairy"], productType = prodTypeArray[randomIntGen(prodTypeArray.length - 1)], findCategory = (productType) => {
    ...
    }, randomNameSelector = () => listType(productType)[randomIntGen(productListLength(productType) - 1)], unitArray = (productType) => {
        if (productType === "fruit") {
            return fruitUnits;
        }
        else if (productType === "milkDairy") {
    ...
    }, randomUnitSelector = () => unitArray(productType)[randomIntGen(unitArray.length - 1)], farmList = await farm.find(), farmListLength = farmList.length, randomFarmSelector = () => {
        const randomNum = randomIntGen(farmListLength - 1);
        const index = randomNum <= 0.9999 ? randomNum : 0;
        return farmList[index]._id;
    }, formList = (productType) => {
        if (productType === "fruit") {
            return fruitForms;
        }
        else if (productType === "milkDairy") {
