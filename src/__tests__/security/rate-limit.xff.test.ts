...
describe("X-Forwarded-For hardening in rate limiting", () => {
  ...
  it("gives independent buckets to different trusted (rightmost) hops", async () => {
    await assertRateLimit(
      reqWithXFF("10.0.0.1, 198.51.100.10"),
      "xff-distinct",
      { limit: 1, windowMs: 60_000 },
    );
    // Different rightmost hop -> fresh bucket -> allowed.
    await expect(
      assertRateLimit(reqWithXFF("10.0.0.2, 198.51.100.11"), "xff-distinct", {
        limit: 1,
        windowMs: 60_000,
      }),
  ...
  it("falls back to x-real-ip when x-forwarded-for is absent", async () => {
    await assertRateLimit(reqWithXFF("ignored", "203.0.113.20"), "xff-realip", {
      limit: 1,
      windowMs: 60_000,
    });
    await expect(
      assertRateLimit(reqWithXFF("ignored", "203.0.113.20"), "xff-realip", {
        limit: 1,
        windowMs: 60_000,
      }),
    ).rejects.toThrow();
